...
export class StressTest {
  ...
  async runTest(): Promise<StressTestResult> {
    ...
    systemBehavior.push(`Baseline: ${baselineAvgResponseTime.toFixed(0)}ms avg response, ${(baselineErrorRate * 100).toFixed(1)}% error rate`);
    ...
    for (const level of STRESS_TEST_LEVELS) {
      ...
      try {
        ...
        console.log(`📊 Results: ${(analysis.successRate * 100).toFixed(1)}% success, ${analysis.avgResponseTime.toFixed(0)}ms avg response`);
        console.log(`🔍 System state: ${analysis.isBroken ? '❌ BROKEN' : analysis.isUnstable ? '⚠️  UNSTABLE' : '✅ STABLE'}`);
  ...
  private async checkSystemHealth(): Promise<{ isHealthy: boolean; details: string }> {
    try {
      ...
      const browserPoolHealthy = data.browserPool && 
  ...
  private printStressReport(result: StressTestResult): void {
    ...
    console.log(`   Response Time: ${result.degradationPoints.responseTime || 'Not reached'} sessions`);
    console.log(`   Error Rate: ${result.degradationPoints.errorRate || 'Not reached'} sessions`);
    console.log(`   Resource Exhaustion: ${result.degradationPoints.resourceExhaustion || 'Not reached'} sessions`);
    ...
    if (result.breakingPoint > 0) {
      console.log(`• System can handle up to ${result.breakingPoint - 1} concurrent sessions under stress`);
    }
    if (result.degradationPoints.responseTime > 0) {
      console.log(`• Response time degradation begins at ${result.degradationPoints.responseTime} sessions`);
    }
    if (result.degradationPoints.errorRate > 0) {
      console.log(`• Error rate increases significantly at ${result.degradationPoints.errorRate} sessions`);
    }
    if (result.degradationPoints.resourceExhaustion > 0) {
      console.log(`• Resource exhaustion occurs at ${result.degradationPoints.resourceExhaustion} sessions`);
    }
    ...
    if (result.breakingPoint > 0) {
      ...
      console.log(`• Set production limit to ${safeLimit} concurrent sessions (70% of breaking point)`);
    }
