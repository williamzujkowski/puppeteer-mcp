      ...
      const workerBypassTests = [
        ...
        async () => {
          const result = await page.evaluate(() => {
            try {
              const blob = new Blob(['postMessage("bypassed")'], { type: 'application/javascript' });
        ...
        async () => {
          const result = await page.evaluate(() => {
            try {
              const blob = new Blob(['onconnect = e => e.ports[0].postMessage("bypassed")'], { type: 'application/javascript' });
