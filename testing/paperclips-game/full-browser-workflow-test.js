  ...
  async createAuthenticatedSession() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.log(`API session creation failed: ${response.status} - ${JSON.stringify(response.data)}`, 'error');
    ...
    const gameElementChecks = [
      {
        ...
        selector: 'button[id*="paperclip"], input[type="button"][value*="paperclip"], #btnMakePaperclip',
