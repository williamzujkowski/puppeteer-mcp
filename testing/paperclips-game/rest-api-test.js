...
class RestApiTest {
  ...
  async testHealthChecks() {
    ...
    try {
      ...
      if (healthResponse.success) {
        ...
        this.log(`Basic health check failed: ${healthResponse.error || healthResponse.data}`, 'error');
      }
      ...
      if (livenessResponse.success) {
        ...
        this.log(`Liveness probe failed: ${livenessResponse.error || livenessResponse.data}`, 'error');
      }
      ...
      if (readinessResponse.success) {
        ...
        this.log(`Readiness probe failed: ${readinessResponse.error || readinessResponse.data}`, 'error');
      }
    ...
    try {
      ...
      if (this.results.token) {
        ...
        if (sessionResponse.success) {
          ...
          this.log(`Token validation failed: ${sessionResponse.error || sessionResponse.data}`, 'error');
        }
      }
  ...
  async testContextManagement() {
    ...
    try {
      ...
      if (contextResponse.success) {
        ...
        this.log(`Context created successfully: ${this.results.contextManagement.contextId}`, 'success');
        
        // Verify context retrieval
        const getContextResponse = await this.makeApiRequest('GET', `/contexts/${this.results.contextManagement.contextId}`, null, {
          'Authorization': `Bearer ${this.results.token}`
        });
        ...
        if (getContextResponse.success) {
          ...
          this.log(`Context retrieval failed: ${getContextResponse.error || getContextResponse.data}`, 'error');
        }
  ...
  async testNavigate(contextId, pageId) {
    ...
    try {
      ...
      const response = await this.makeApiRequest('POST', `/contexts/${contextId}/execute`, navigationData, {
        'Authorization': `Bearer ${this.results.token}`
      });
  ...
  async testScreenshot(contextId, pageId, label) {
    ...
    try {
      ...
      const response = await this.makeApiRequest('POST', `/contexts/${contextId}/execute`, screenshotData, {
        'Authorization': `Bearer ${this.results.token}`
      });
      ...
      if (response.success && response.data.data.data) {
        ...
        this.log(`Screenshot capture (${label}) failed: ${this.results.browserActions[resultKey].error}`, 'error');
      }
  ...
  async testContentExtraction(contextId, pageId) {
    ...
    try {
      ...
      const extractData = {
        type: 'evaluate',
        pageId: pageId,
        function: '() => ({ title: document.title, body: document.body.innerHTML.substring(0, 1000), url: window.location.href })'
      };

      const response = await this.makeApiRequest('POST', `/contexts/${contextId}/execute`, extractData, {
        'Authorization': `Bearer ${this.results.token}`
      });
      ...
      if (response.success) {
        ...
        this.log(`Content extraction failed: ${this.results.browserActions.content.error}`, 'error');
      }
  ...
  async testClick(contextId, pageId) {
    ...
    try {
      ...
      const buttonResponse = await this.makeApiRequest('POST', `/contexts/${contextId}/execute`, findButtonData, {
        'Authorization': `Bearer ${this.results.token}`
      });
      ...
      if (buttonResponse.success && buttonResponse.data.data.data.found) {
        ...
        const clickResponse = await this.makeApiRequest('POST', `/contexts/${contextId}/execute`, clickData, {
          'Authorization': `Bearer ${this.results.token}`
        });
        ...
        if (buttonResponse.data?.data?.data?.availableButtons) {
          this.log(`Available buttons: ${JSON.stringify(buttonResponse.data.data.data.availableButtons)}`, 'info');
        }
      }
  ...
  async calculateMetrics() {
    if (this.results.apiMetrics.requestTimes.length > 0) {
      ...
      this.results.apiMetrics.averageResponseTime = totalTime / this.results.apiMetrics.requestTimes.length;
    }
  }
  ...
  async cleanup() {
    ...
    try {
      ...
      if (this.results.contextManagement.contextId && this.results.token) {
        const deleteResponse = await this.makeApiRequest('DELETE', `/contexts/${this.results.contextManagement.contextId}`, null, {
          'Authorization': `Bearer ${this.results.token}`
        });
        ...
        if (deleteResponse.success) {
          ...
          this.log(`Failed to delete test context: ${deleteResponse.error || deleteResponse.data}`, 'warning');
        }
      }
  ...
  printSummary() {
    ...
    console.log(`Average Response Time: ${Math.round(this.results.apiMetrics.averageResponseTime)}ms`);
    ...
    console.log(`✅ Health Checks: ${Object.values(this.results.healthChecks).every(Boolean) ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Authentication: ${this.results.authentication.tokenGeneration && this.results.authentication.tokenValidation ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Session Management: ${this.results.sessionManagement.sessionCreation && this.results.sessionManagement.sessionRetrieval ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Context Management: ${this.results.contextManagement.contextCreation && this.results.contextManagement.contextRetrieval ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Browser Navigation: ${this.results.browserActions.navigate.success ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Screenshot Capture: ${this.results.browserActions.screenshot.success ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Content Extraction: ${this.results.browserActions.content.success ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Click Interaction: ${this.results.browserActions.click.success ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Second Screenshot: ${this.results.browserActions.secondScreenshot.success ? 'PASS' : 'FAIL'}`);
