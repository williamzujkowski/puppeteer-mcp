  ...
  navigationConflicts: [
    {
      ...
      operations: [
        ...
        { action: 'navigate', params: { url: 'https://williamzujkowski.github.io/paperclips/index2.html' } },
  ...
  resourceContention: [
    {
      ...
      setup: { url: 'https://williamzujkowski.github.io/paperclips/index2.html' },
      operations: Array(5).fill().map(() => ({ action: 'screenshot' })),
      expectedBehavior: 'all_succeed'
    },
    {
      name: 'concurrent_evaluations',
      setup: { url: 'https://williamzujkowski.github.io/' },
      operations: Array(10).fill().map((_, i) => ({
        action: 'evaluate',
...
async function testConcurrentScenario(sessionId, test, category) {
  ...
  try {
    ...
    result.analysis = {
      ...
      averageResponseTime: result.operations.reduce((sum, op) => sum + op.responseTime, 0) / result.operations.length
    };
    ...
    switch (test.expectedBehavior) {
      case 'last_wins':
        ...
        result.success = fulfilled.length === 1 || 
                        (fulfilled.length > 1 && fulfilled[fulfilled.length - 1].index === test.operations.length - 1);
        break;
        
      case 'all_succeed':
        result.actualBehavior = fulfilled.length === test.operations.length ? 'all_succeed' : 'partial_success';
      ...
      case 'sequential_execution':
        ...
        const isSequential = responseTimes.every((time, i) => 
          i === 0 || time > responseTimes[i - 1] * 0.8
        );
      ...
      case 'graceful_cleanup':
        result.actualBehavior = rejected.some(op => op.error?.includes('closed') || op.error?.includes('terminated')) 
          ? 'graceful_cleanup' 
          : 'unexpected_behavior';
...
async function testRaceConditions(sessionId) {
  ...
  try {
    ...
    const navPromises = [
      axios.post(`${API_BASE}/contexts/${sessionId}/action`, {
        sessionId,
        action: 'navigate',
    ...
    await axios.post(`${API_BASE}/contexts/${sessionId}/action`, {
      sessionId,
      action: 'navigate',
    ...
    const clickPromises = Array(5).fill().map(() =>
      axios.post(`${API_BASE}/contexts/${sessionId}/action`, {
        sessionId,
        action: 'click',
    ...
    for (const [category, tests] of Object.entries(CONCURRENT_TESTS)) {
      ...
      for (const test of tests) {
        ...
        if (result.success) {
          ...
          console.log(`  └─ Expected: ${result.expectedBehavior}, Actual: ${result.actualBehavior}`);
          console.log(`  └─ ${result.analysis.fulfilled}/${result.analysis.totalOperations} operations succeeded`);
        } else {
