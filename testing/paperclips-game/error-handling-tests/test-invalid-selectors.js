...
async function testInvalidSelector(sessionId, test, category) {
  ...
  try {
    ...
    result.success = error.message?.includes('selector') || error.message?.includes('element');
  }
    ...
    for (const interaction of interactions) {
      ...
      if (response.status >= 400) {
        ...
        interactionResult.success =
          interactionResult.error?.includes(interaction.expectedError) || response.status >= 400;
      }
...
async function runTests() {
  ...
  try {
    ...
    results.interactions.interactions?.forEach((interaction) => {
      console.log(
        `  - ${interaction.name}: ${interaction.success ? '✓' : '✗'} ${interaction.error || ''}`,
      );
    });
    ...
    results.timeouts.timeouts?.forEach((timeout) => {
      console.log(
        `  - ${timeout.timeout}ms timeout: actual ${timeout.actualTime}ms (${timeout.withinTolerance ? '✓' : '✗'})`,
      );
    });
