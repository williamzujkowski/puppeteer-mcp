  ...
  try {
    ...
    if (scriptResult.status >= 400) {
      ...
      if (collectedErrors.data.data?.result) {
        ...
        result.success = allErrors.includes(test.expectedError) || 
                        (category === 'asyncErrors' && result.pageErrors.some(e => e.type === 'unhandledrejection'));
      }
    }
  ...
  try {
    for (const errorType of errorTypes) {
      ...
      if (errorResult.status >= 400) {
        ...
        errorDetail.correctType = errorMessage.includes(errorType.type) || 
                                 errorCode.includes(errorType.type);
        errorDetail.hasStackTrace = errorMessage.includes('at ') || 
                                   errorResult.data.stack !== undefined;
        errorDetail.hasLineNumber = errorMessage.match(/line \d+/) !== null ||
                                   errorResult.data.line !== undefined;
      }

      result.errorTypes.push(errorDetail);
    }

    result.success = result.errorTypes.every(err => 
      err.reported && err.correctType
    );
...
async function runTests() {
  ...
  try {
    ...
    results.isolation.contexts?.forEach((ctx, idx) => {
      console.log(`  - Context ${idx}: Error=${ctx.errorCaused ? '✓' : '✗'}, Functional=${ctx.functionalAfterError ? '✓' : '✗'}`);
    });
    ...
    results.reporting.errorTypes?.forEach(err => {
      console.log(`  - ${err.type}: Reported=${err.reported ? '✓' : '✗'}, Type=${err.correctType ? '✓' : '✗'}, Stack=${err.hasStackTrace ? '✓' : '✗'}`);
    });
