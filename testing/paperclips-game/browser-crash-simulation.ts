...
async function getBrowserProcessIds(): Promise<number[]> {
  try {
    ...
    const { stdout } = await execAsync("ps aux | grep -E 'chrome|chromium' | grep -v grep | awk '{print $2}'");
...
class PerformanceMonitor {
  ...
  getReport(): void {
    ...
    console.log(`  Total Browsers - Min: ${Math.min(...totalBrowsers)}, Max: ${Math.max(...totalBrowsers)}, Avg: ${(totalBrowsers.reduce((a, b) => a + b, 0) / totalBrowsers.length).toFixed(1)}`);
    console.log(`  Active Browsers - Min: ${Math.min(...activeBrowsers)}, Max: ${Math.max(...activeBrowsers)}, Avg: ${(activeBrowsers.reduce((a, b) => a + b, 0) / activeBrowsers.length).toFixed(1)}`);
    console.log(`  Queued Requests - Min: ${Math.min(...queuedRequests)}, Max: ${Math.max(...queuedRequests)}, Avg: ${(queuedRequests.reduce((a, b) => a + b, 0) / queuedRequests.length).toFixed(1)}`);
