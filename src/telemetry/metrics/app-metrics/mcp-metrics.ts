...
export class McpMetricsImpl implements McpMetrics {
   ...
   * Record MCP request
   */
  recordMcpRequest(method: string, duration: number, success: boolean): void {
    const labels = {
      method,
   ...
   * Record MCP error
   */
  recordMcpError(errorType: string, method: string, severity: string = 'error'): void {
   ...
   */
  recordMcpProtocolEvent(eventType: string, direction: 'inbound' | 'outbound'): void {
