...
export class ConcurrentSessionTest {
  ...
  async runTest(maxConcurrentSessions: number = 10): Promise<PerformanceTestResult> {
    ...
    console.log(
      `📊 Sessions: ${result.summary.successfulSessions}/${result.summary.totalSessions} successful`,
    );
  ...
  private async runSession(sessionIndex: number, token: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ Session ${sessionIndex + 1} completed in ${PerformanceUtils.formatDuration(sessionMetrics.duration)}`,
      );
    } catch (error) {
  ...
  private async runSessionActions(
    ...
    const actions = [
      ...
      {
        action: 'screenshot',
        params: { filename: `session-${sessionMetrics.sessionId}-gameplay.png` },
      },
      { action: 'navigate', params: { url: this.config.testUrls[1] } },
      { action: 'wait', params: { selector: 'body', timeout: 5000 } },
      {
        action: 'screenshot',
        params: { filename: `session-${sessionMetrics.sessionId}-homepage.png` },
      },
    ];
    ...
    for (const { action, params } of actions) {
      try {
        ...
        if (result.success) {
          sessionMetrics.maxResponseTime = Math.max(
            sessionMetrics.maxResponseTime,
            result.duration,
          );
          sessionMetrics.minResponseTime = Math.min(
            sessionMetrics.minResponseTime,
            result.duration,
          );
        } else {
          sessionMetrics.errorCount++;
  ...
  private startMonitoring(): void {
    this.monitoringInterval = setInterval(async () => {
      try {
        ...
        const browserPoolMetrics = await PerformanceUtils.getBrowserPoolMetrics(
          this.config.baseUrl,
        );
        ...
        // Log current status
        if (this.systemMetrics.length % 10 === 0) {
          // Log every 10 seconds
          console.log(
            `📊 Active sessions: ${this.sessions.size}, CPU: ${systemMetrics.cpuUsage.percent.toFixed(1)}%, Memory: ${PerformanceUtils.formatBytes(systemMetrics.memoryUsage.heapUsed)}`,
          );
        }
      } catch (error) {
  ...
  private calculateResults(): PerformanceTestResult {
    ...
    const maxConcurrentSessions = Math.max(
      ...this.systemMetrics.map((m) => m.browserPoolMetrics.activeBrowsers),
    );
