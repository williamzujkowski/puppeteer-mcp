...
export class ScalabilityTest {
  ...
  async runTest(): Promise<ScalabilityTestResult> {
    ...
    for (const sessionCount of sessionCounts) {
      ...
      try {
        ...
        console.log(
          `   Avg Response Time: ${(responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length) | 0}ms`,
        );
  ...
  private generateScalabilityRecommendations(
    ...
    if (maxSustainable < 5) {
      recommendations.push(
        'System shows low scalability - consider optimizing browser pool configuration',
      );
    ...
    if (degradationPoint > 0) {
      recommendations.push(
        `Performance degradation starts at ${degradationPoint} concurrent sessions`,
      );
  ...
  private printScalabilityReport(result: ScalabilityTestResult): void {
    ...
    console.log(
      `📉 Performance Degradation Point: ${result.performanceDegradationPoint || 'Not reached'}`,
    );
