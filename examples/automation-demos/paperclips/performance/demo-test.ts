...
export class DemoPerformanceTest {
  ...
  async runDemoTest(): Promise<PerformanceTestResult> {
    ...
    console.log(
      `📊 Results: ${result.summary.successfulSessions}/${result.summary.totalSessions} sessions successful`,
    );
  ...
  private async simulateSession(sessionIndex: number): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ Demo session ${sessionIndex + 1} completed in ${PerformanceUtils.formatDuration(sessionMetrics.duration)}`,
      );
    } catch (error) {
      sessionMetrics.errorCount++;
      sessionMetrics.errors.push({
  ...
  private calculateResults(): PerformanceTestResult {
    ...
    recommendations.push(
      'Actual performance may vary based on server configuration and network conditions',
    );
