...
async function rollbackRelease() {
  ...
  try {
    ...
    execSync(`git checkout -b ${rollbackBranch} ${targetTag}`, { cwd: projectRoot, stdio: 'inherit' });
    ...
    try {
      ...
      console.log(`${colors.yellow}Warning: Could not deprecate on NPM. You may need to do this manually.${colors.reset}`);
    }
    ...
    try {
      ...
      console.log(`${colors.red}Failed to publish. You may need to do this manually.${colors.reset}`);
    }
...
`;
    ...
    execSync(`git commit -m "rollback: v${currentVersion} to ${targetTag}\n\nReason: ${rollbackPackageJson.rollback.reason}"`, { cwd: projectRoot });
