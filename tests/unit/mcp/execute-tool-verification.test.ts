...
describe('Execute-in-Context Tool Implementation', () => {
  it('should have the execute-in-context tool defined in the tools array', () => {
    ...
    expect(serverContent).toContain("contextId: { type: 'string', description: 'Context ID to execute command in' }");
    expect(serverContent).toContain("command: { type: 'string', description: 'Command to execute' }");
    expect(serverContent).toContain("parameters: { type: 'object', description: 'Parameters for the command' }");
