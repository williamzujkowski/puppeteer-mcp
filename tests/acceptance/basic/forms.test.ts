...
import { retryOperation, validateUrl, TestData, AssertionHelpers, PerformanceTracker } from '../utils/test-helpers.js';
...
describe('Form Interaction Tests', () => {
  ...
  describe('Complex Form Interactions', () => {
    it('should fill a complex form with various input types', async () => {
      // Skip if DemoQA is not accessible (it sometimes has issues)
      const isAccessible = await validateUrl(TEST_TARGETS.testing.demoQA + '/automation-practice-form');
      if (!isAccessible) {
      ...
      await retryOperation(async () => {
        await mcpNavigate(mcpClient.client, sessionInfo.contextId, TEST_TARGETS.testing.demoQA + '/automation-practice-form');
        ...
        await mcpType(mcpClient.client, sessionInfo.contextId, '#currentAddress', '123 Test Street\\nTest City, TC 12345');
  ...
  describe('Dynamic Form Elements', () => {
    it('should handle dynamic content loading', async () => {
      await retryOperation(async () => {
        await mcpNavigate(mcpClient.client, sessionInfo.contextId, TEST_TARGETS.testing.theInternet + '/dynamic_loading/1');
    ...
    it('should wait for elements to become visible', async () => {
      await retryOperation(async () => {
        await mcpNavigate(mcpClient.client, sessionInfo.contextId, TEST_TARGETS.testing.theInternet + '/dynamic_loading/2');
